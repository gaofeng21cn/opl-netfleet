...
- `nikki-mixin.yaml`：由用户维护的原生 Nikki mixin，只补充目标网络或特定 provider 所需的 DNS 兼容和关键域名规则；通用的 WAN DHCP 本地主 DNS、境外规则集加密解析、代理节点加密解析和 GeoIP 过滤 fallback 基线由内置 `bundle:base-v1` 提供。

...
export function descriptor_error(value, id) {
	...
	if (value.schema == 'opl-netfleet-service-plugin.v1') {
		for (let key in keys(value)) if (index(['schema','id','label','version','api_version','package','services','commands','package_dependencies','lifecycle'], key) < 0) return 'plugin_manifest_invalid';
		if (type(value.services) != 'object' || !length(value.services) || type(value.commands) != 'object') return 'plugin_manifest_invalid';
		...
		if (value.lifecycle != null) {
			...
			for (let action, hook in value.lifecycle) if (index(['drain','resume'], action) < 0 || type(hook) != 'object' ||

...
let shell, capture, parsed_command, gateway, directory, parent, fail, add_missing, add_resource, resource_paths, discovery, safe_discovery, get, owner, save_file, restore_file, remove_work, stage, rollback, apply, command_migration_get, command_migration_apply;

const stop_previous = context.use("mihomo.backend").stop;
...
const migrate_sections = context.use("setup.migration-model").migrate_sections;
const profile_path = context.use("setup.migration-model").profile_path;
const relative_path = context.use("setup.migration-model").relative_path;
...
parent = function(path) { const parts = split(path, "/"); pop(parts); return join("/", parts); };
fail = function(error, detail) { return { ok: false, error: error, result: detail ?? null }; };
add_missing = function(found, reason) {
	if (index(found.missing, reason) < 0) push(found.missing, reason);
};

add_resource = function(found, source, destination, optional) {
...
resource_paths = function(found, value) {
...
discovery = function() {
	...
	found.revision = sha256_text(sprintf("%J", { uci: sha256("/etc/config/nikki"), policy: sha256(POLICY_PATH),
		code: map(filter(context.inventory(), item => index(["setup", "mihomo", "platform", "models"], item.id) >= 0),
			item => ({ id: item.id, revision: item.revision })), init: sha256("/etc/init.d/opl-netfleet-core"),
...
rollback = function(snapshot, work, installed) {
...
command_migration_get = function(argv) {
	return get();
};

command_migration_apply = function(argv) {
	return apply(argv[1]);
};

return { get, apply, command_migration_get, command_migration_apply };
};

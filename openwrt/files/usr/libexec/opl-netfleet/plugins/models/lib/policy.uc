...
let is_object, is_nonempty_string, is_stable_id, is_stable_uci_section, add_error, validate_profile_reference, validate_bundle_reference, has, valid_mode, array_has, validate_margin, canonical_cidr, validate_routing_rules, capability_before, region_before, ordered_regions, ordered_capabilities, validate_region_list, validate_checks, automation, guard_probe_url, resolved_margin, region_switch_margin, leaf_switch_margin, validate;
...
is_stable_id = function(value) {
	return is_nonempty_string(value) && match(value, /^[A-Za-z0-9][A-Za-z0-9_-]*$/);
};

is_stable_uci_section = function(value) {
	return is_nonempty_string(value) && match(value, /^[A-Za-z0-9_]+$/) &&
		!match(value, /^cfg[0-9a-f]+$/);
};

add_error = function(errors, message) {
	push(errors, message);
};

validate_profile_reference = function(value, path, errors) {
...
validate_bundle_reference = function(value, path, errors) {
...
valid_mode = function(value, allowed) {
...
validate_routing_rules = function(rules, capabilities, errors) {
...
capability_before = function(policy, left, right) {
...
ordered_capabilities = function(policy) {
...
validate_region_list = function(value, path, regions, errors) {
	if (value == null) {
		return;
	}
...
validate_checks = function(checks, errors) {
	if (checks == null) {
		return;
	}
	if (!is_object(checks)) {
...
guard_probe_url = function(policy) {
...
resolved_margin = function(policy, capability, field) {
...
region_switch_margin = function(policy, capability) {
	return resolved_margin(policy, capability, "region_switch_margin_ms");
};

leaf_switch_margin = function(policy, capability) {
...
return { canonical_cidr, validate_routing_rules, ordered_regions, ordered_capabilities, automation, guard_probe_url, region_switch_margin, leaf_switch_margin, validate };
};

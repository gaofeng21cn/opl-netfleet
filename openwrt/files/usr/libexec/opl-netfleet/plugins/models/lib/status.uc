...
let evidence_delay, current_evidence, aggregate_evidence, follow_path, candidate_by_group, provider_by_group, region_by_group, binding_groups, business_routes, binding_entry, provider_nodes, group_nodes, has_available_node, last_leaf, path_alive, native_path_for_capability, native_capability_runtime, native_runtime, direct_name, is_direct_terminal, path_has_direct, resolve_runtime, build;

const identity_matches = context.use("models.evidence").identity_matches;
...
const is_proxy_leaf = context.use("models.selector").is_proxy_leaf;
...
current_evidence = function(evidence, policy, manifest, capability) {
...
aggregate_evidence = function(evidence, dimension, id) {
...
follow_path = function(proxy_state, start) {
	const path = [];
	const seen = {};
	let current = start;
...
candidate_by_group = function(entry, group) {
...
provider_by_group = function(entry, group) {
...
region_by_group = function(entry, group) {
...
binding_groups = function(policy, capability) {
	const result = [];
...
business_routes = function(entry, proxy_state) {
...
binding_entry = function(policy, capability) {
	const names = keys(policy?.bindings ?? {});
	for (let i = 0; i < length(names); i++) {
...
provider_nodes = function(provider_state, source_name) {
...
group_nodes = function(proxy_state, provider_state, source_name, group_name) {
...
has_available_node = function(nodes) {
...
last_leaf = function(proxy_state, path) {
	if (length(path) == 0) {
		return null;
	}
...
path_alive = function(proxy_state, path) {
...
native_path_for_capability = function(native, capability) {
...
native_capability_runtime = function(native, capability) {
...
native_runtime = function(policy, state, owner) {
	const result = {
...
direct_name = function(entry) {
	return entry?.direct_name ?? "DIRECT";
};

is_direct_terminal = function(proxy_state, value, entry) {
	return value == direct_name(entry) &&
		lc(`${proxy_state?.[value]?.type ?? ""}`) == "direct";
};

path_has_direct = function(proxy_state, path, entry) {
	for (let i = 0; i < length(path); i++) {
...
resolve_runtime = function(entry, state) {
	const proxy_state = state?.proxies ?? {};
...
build = function(policy, manifest, state, evidence, owner) {
	const proxy_state = state?.proxies ?? {};
	const provider_state = state?.providers ?? {};
	const capabilities = [];
	const providers = {};

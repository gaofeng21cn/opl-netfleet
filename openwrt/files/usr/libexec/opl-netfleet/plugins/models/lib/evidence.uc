...
let clone, valid_identity, measurement_identity, identity_matches, same_identity, best_by, valid_aggregate, aggregate_ids, initial_aggregates, add_aggregate_samples, entries_for, decision_for, legacy_snapshot, selection_snapshot, valid_decision, validate_entries, validate_aggregates, validate;



const MEASUREMENT_MODEL = "all_capability_groups_v2";
...
valid_identity = function(identity) {
...
measurement_identity = function(policy, manifest) {
...
identity_matches = function(left, right) {
...
best_by = function(entries, field) {
	const result = {};
...
valid_aggregate = function(entry) {
...
aggregate_ids = function(entries, field) {
	const result = {};
	for (let i = 0; i < length(entries); i++) {
...
initial_aggregates = function(previous, current_ids) {
	const result = {};
...
add_aggregate_samples = function(aggregates, latest, sampled_at) {
...
entries_for = function(candidates, sampled_at) {
	const entries = [];
...
decision_for = function(capability, decision) {
	return {
...
legacy_snapshot = function(candidates, capability, decision, probes) {
...
selection_snapshot = function(previous, candidates, capability, decision, probes, identity, groups) {
...
valid_decision = function(decision) {
...
validate_entries = function(entries, prefix) {
...
validate_aggregates = function(aggregates, prefix, dimension) {
...
return { MEASUREMENT_MODEL, measurement_identity, identity_matches, selection_snapshot, validate };
};

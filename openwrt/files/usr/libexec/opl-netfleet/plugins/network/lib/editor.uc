...
const project = context.use("network.model").project;
const public_settings = context.use("network.model").public_settings;
const validate_request = context.use("network.model").validate_request;
const runtime_profile = context.use("network.model").runtime_profile;
const error_code = context.use("network.model").error_code;
...
discover = function() {
	...
	const rendered = command_json(`ucode ${shell_quote(GATEWAY)} native-gateway-preview`);
...
set_value = function(uci, section, field, value) {
...
install_file = function(source, destination) {
...
restore_files = function(snapshot) {
	let restored = true;
...
command_network_get = function(argv) {
	return get(argv[1]);
};

command_network_validate = function(argv) {
	return validate(argv[1]);
};

command_network_apply = function(argv) {
	return apply(argv[1]);
};

return { get, validate, apply, command_network_get, command_network_validate, command_network_apply };
};

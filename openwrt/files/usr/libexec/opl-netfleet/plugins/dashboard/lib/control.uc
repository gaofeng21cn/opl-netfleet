...
let inspection, capture, shell, version_valid, error_code, cache_directory, configuration, safe_directory, installed, checked, resource, get, check, archive_valid, tree_valid, remove_owned, update, dispatch, command_dashboard_get, command_dashboard_check, command_dashboard_update;

const bundled_version = context.use("dashboard.version").bundled_version;
...
version_valid = function(value) { return type(value) == "string" && match(value, /^v[0-9]+\.[0-9]+\.[0-9]+$/); };
error_code = function(error) {
	const code = trim(split(`${error}`, "\n")[0]);
	return match(code, /^dashboard_[a-z_]+$/) ? code : "dashboard_update_failed";
};
cache_directory = function() { return fs.lstat(CACHE_DIR) == null ? fs.mkdir(CACHE_DIR, 0700) : private_directory(CACHE_DIR); };
...
checked = function() { return private_file(CACHE) ? read_json(CACHE) : null; };
...
tree_valid = function(path, depth) {
...
remove_owned = function(path) { return fs.lstat(path) == null || fs.lstat(path)?.type == "directory" && shell(`rm -rf ${q(path)}`); };
...
dispatch = function(action, argument) {
...
command_dashboard_get = function(argv) {
...
command_dashboard_check = function(argv) {
	const observed = inspection();
	const error = admission(extension, observed, argv[0], KIND);
	if (error != null) return { ok: false, error: error };
	return dispatch("check", argv[1]);
};

command_dashboard_update = function(argv) {
	const observed = inspection();
	const error = admission(extension, observed, argv[0], KIND);
	if (error != null) return { ok: false, error: error };
	return dispatch("update", argv[1]);
};

return { extension, inspection, resource, get, check, update, dispatch, command_dashboard_get, command_dashboard_check, command_dashboard_update };
};

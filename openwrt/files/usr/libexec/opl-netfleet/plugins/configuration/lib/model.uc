...
let object, nonempty, stable_id, clone, has, sorted_names, known_keys, option_exists, option_by_id, source_groups, joined, next_display_order, array_of_known_ids, probe_url, set_probe_url, allowed_region_ids, display_for_ref, project, validate_request, apply, add_change, singular_scope, changes;

const validate_policy = context.use("models.policy").validate;
const ordered_regions = context.use("models.policy").ordered_regions;
const validate_routing_rules = context.use("models.policy").validate_routing_rules;
...
known_keys = function(value, allowed, errors, path) {
	if (!object(value)) {
...
option_exists = function(options, kind, ref) {
...
option_by_id = function(options, id) {
	for (let i = 0; i < length(options ?? []); i++) {
...
next_display_order = function(items) {
	let highest = 0;
...
array_of_known_ids = function(value, known, errors, path) {
	if (type(value) != "array") {
...
probe_url = function(policy, probe_id) {
...
set_probe_url = function(policy, probe_id, url) {
	const probes = policy?.fail_open?.probes ?? [];
	for (let i = 0; i < length(probes); i++) {
...
allowed_region_ids = function(policy, capability) {
...
display_for_ref = function(options, kind, ref, fallback) {
	for (let i = 0; i < length(options ?? []); i++) {
...
project = function(policy, resources) {
	const providers = [];
...
validate_request = function(policy, request, resources) {
	const errors = [];
...
apply = function(policy, request, resources) {
...
return { project, validate_request, apply, changes };
};

...
let failure, capture, parsed, shell, gateway, native_ready, directory, parent, prepare_directory, readable_input, safe_target, profile_entries, revision, input, get, profile_get, validation_paths, test_candidate, stage_input, collect_files, profile_save, profile_delete, sections, backup_export, snapshot, wait_runtime, restore_choices, owner, resume, restore_snapshot, clean_work, stage_config, backup_restore, core_action, diagnostics, command_maintenance_get, command_profile_get, command_profile_save, command_profile_delete, command_backup_export, command_backup_restore, command_core_action, command_diagnostics_get;

const BACKUP_FORMAT = context.use("maintenance.model").BACKUP_FORMAT;
const MAX_PROFILE_BYTES = context.use("maintenance.model").MAX_PROFILE_BYTES;
const MAX_BACKUP_BYTES = context.use("maintenance.model").MAX_BACKUP_BYTES;
const profile_id = context.use("maintenance.model").profile_id;
const file_path = context.use("maintenance.model").file_path;
const profile_referenced = context.use("maintenance.model").profile_referenced;
const validate_backup = context.use("maintenance.model").validate_backup;
const redact_line = context.use("maintenance.model").redact_line;
...
const validate_policy = context.use("platform.documents").validate_policy;
...
gateway = function() { return parsed(`ucode ${q(GATEWAY)} native-gateway-status`)?.result; };
native_ready = function() { return KIND == "native-mihomo" && private_directory(BASE) && private_file(CONFIG); };
directory = function(path) {
	const info = fs.lstat(path);
	return info == null ? fs.mkdir(path, 0700) : info.type == "directory" && info.uid == 0 && (info.mode & 022) == 0;
};
parent = function(path) { return join("/", slice(split(path, "/"), 0, -1)); };
prepare_directory = function(path) {
	if (path == ROOT) return fs.lstat(ROOT)?.type == "directory";
	return index(path, `${ROOT}/`) == 0 && prepare_directory(parent(path)) && directory(path);
};
readable_input = function(path) {
...
validation_paths = function(value, work) {
...
test_candidate = function(path, work) {
...
stage_input = function(work, relative, content) {
...
restore_choices = function(before, exact) {
...
resume = function(before, work, exact) {
...
restore_snapshot = function(before, work) {
...
clean_work = function(work) { return shell(`rm -rf ${q(work)}`); };
stage_config = function(backup, before, work) {
...
command_maintenance_get = function(argv) {
	return get(argv[1]);
};

command_profile_get = function(argv) {
	return profile_get(argv[1]);
};

command_profile_save = function(argv) {
	return profile_save(argv[1]);
};

command_profile_delete = function(argv) {
	return profile_delete(argv[1]);
};

command_backup_export = function(argv) {
	return backup_export(argv[1]);
};

command_backup_restore = function(argv) {
	return backup_restore(argv[1]);
};

command_core_action = function(argv) {
	return core_action(argv[1]);
};

command_diagnostics_get = function(argv) {
	return diagnostics(argv[1]);
};

return { get, profile_get, profile_save, profile_delete, backup_export, backup_restore, core_action, diagnostics, command_maintenance_get, command_profile_get, command_profile_save, command_profile_delete, command_backup_export, command_backup_restore, command_core_action, command_diagnostics_get };
};

...
let clone, group_index, generated_name_conflicts, binding_capability, binding_kind, rewritten_member, rewrite_group_members, rule_target, rewrite_rule, rewrite_rule_list, capability_group_name, inject_routing_rules, legacy_closure, normalize_policy_group, entry_alias, standardize_profile, has, region_display_name, provider_display_name, provider_group_name, provider_tier_name, provider_source_name, region_group_name, region_aggregate_name, region_layer_name, preferred_group_name, proxy_path_group_name, automatic_group_name, direct_guard_group_name, provider_before, ordered_enabled_providers, automatic_capability, capability_generation_order, array_has, push_unique, capability_allows_region, protected_probe, mapping_filter, compile;

const ordered_capabilities = context.use("models.policy").ordered_capabilities;
const ordered_regions = context.use("models.policy").ordered_regions;
const region_switch_margin = context.use("models.policy").region_switch_margin;
const leaf_switch_margin = context.use("models.policy").leaf_switch_margin;
const canonical_cidr = context.use("models.policy").canonical_cidr;
...
group_index = function(groups, name) {
...
generated_name_conflicts = function(groups, name, permitted_index) {
	for (let i = 0; i < length(groups); i++) {
...
binding_capability = function(binding) {
	return binding?.capability;
};

binding_kind = function(binding) {
	return binding?.kind;
};

rewritten_member = function(member, replacements) {
	return replacements[member] ?? member;
};

rewrite_group_members = function(group, replacements) {
...
rewrite_rule = function(rule, replacements) {
...
rewrite_rule_list = function(rules, replacements) {
...
capability_group_name = function(policy, capability) {
	return policy?.capabilities?.[capability]?.display_name ?? capability;
};

inject_routing_rules = function(compiled, policy) {
...
legacy_closure = function(original_groups, entry_group, entry_groups) {
	const known = {};
...
normalize_policy_group = function(original, members) {
	const ordered = [];
...
entry_alias = function(name, target) {
	return {
		name: name,
		type: "select",
		proxies: [target],
...
standardize_profile = function(compiled, original_groups, enabled_bindings, enabled_capabilities, generated_groups) {
	const entry_groups = {};
	const policy_groups = {};
	const replacements = {};
...
region_display_name = function(policy, region) {
...
provider_display_name = function(provider_sources, provider_name) {
	return provider_sources?.[provider_name]?.display_name ?? provider_name;
};

provider_group_name = function(capability_name, provider_name) {
	return `${capability_name} · ${provider_name}`;
};

provider_tier_name = function(capability_name, role) {
	return `${capability_name} · ${role == "primary" ? "主用机场" : "备用机场"}`;
};

provider_source_name = function(provider_name) {
	return `NETFLEET-SOURCE-${provider_name}`;
};

region_group_name = function(capability_name, region_name, provider_name) {
	return `${capability_name} · ${region_name} · ${provider_name}`;
};

region_aggregate_name = function(capability_name, region_name) {
	return `${capability_name} · ${region_name}`;
};

region_layer_name = function(capability_name, region_name, layer) {
	return `${capability_name} · ${region_name} · ${layer == "primary" ? "主用" : "备用"}`;
};

preferred_group_name = function(capability_name) {
	return `${capability_name} · 当前优选`;
};

proxy_path_group_name = function(capability_name) {
	return `${capability_name} · 代理路径`;
};

automatic_group_name = function(capability_name) {
	return `${capability_name} · 自动选优`;
};

direct_guard_group_name = function() {
	return "NetFleet · 直连护栏";
};

provider_before = function(policy, left, right) {
...
ordered_enabled_providers = function(policy) {
	const result = [];
...
automatic_capability = function(policy, capability) {
	return policy?.capabilities?.[capability]?.mode == "automatic";
};

capability_generation_order = function(policy, capabilities) {
	const ordered = [];
	const added = {};
...
capability_allows_region = function(policy, capability, region, automatic) {
...
protected_probe = function(policy, probe_id) {
...
mapping_filter = function(mapping) {
...
compile = function(profile, policy, policy_source_digest, recovery_profile_digest, policy_digest, provider_profiles) {
	...
	for (let binding_index = 0; binding_index < length(generation_capabilities); binding_index++) {
		...
		for (let i = 0; i < length(provider_names); i++) {
			...
			push(compiled_groups, {
				...
				"empty-fallback": "REJECT",
				url: path_probe.url,
				"expected-status": path_probe.head_expected_status ?? path_probe.expected_status,
		...
		if (automatic) {
			push(compiled_groups, {
				...
				"empty-fallback": "DIRECT",
				url: guard_probe.url,
				"expected-status": guard_probe.head_expected_status ?? guard_probe.expected_status,
				timeout: healthcheck.timeout_ms,
				interval: healthcheck.interval_seconds,
				"max-failed-times": healthcheck.max_failed_times,
				lazy: true,

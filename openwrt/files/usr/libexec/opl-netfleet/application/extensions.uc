...
import * as plugins from "./plugins.uc";

// Built-in names remain separate from the dynamically discovered plugin namespace.
const MODULES = [compatibility, dashboard];

export function dispatch(command, envelope) {
	const plugin_result = plugins.dispatch(command, envelope);
...
export function inventory(versions, resources) {
	return [ ...map(MODULES, module => component(module.extension,
		module.inspection(resources?.[module.extension.id]), versions, KIND)), ...plugins.inventory(versions) ];
};

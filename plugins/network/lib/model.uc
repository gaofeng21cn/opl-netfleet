...
error_code = function(error, fallback) {
...
project = function(profile, sections) {
	const by_name = {};
...
boolean = function(value, errors, path) {
	if (type(value) != "bool") push(errors, { path: path, reason: "boolean_required" });
};
bounded_array = function(value, errors, path, maximum) {
...
validate_policies = function(value, errors, path) {
...
validate_request = function(request, revision, current, resources) {
	const errors = [];
...
return { error_code, project, public_settings, validate_request, managed_policy, runtime_profile };
};

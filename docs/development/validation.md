...
`ucode tests/benchmark-management.uc <runtime源码目录>` 在临时目录中使用真实宿主和
文件摘要实现，测量 1、16、64 个已安装插件下的普通命令分派，以及同一服务上下文中
未变化 YAML 的重复读取。对照时以相同脚本顺序运行旧、新不可变源码，保留原始 JSON；
不并行运行两组计时。原生 OpenWrt 资格也执行该场景，并在 native 回执中保存结果。
这些数值只表示管理路径耗时，不代表整机 CPU、真实状态接口延迟或网络吞吐。
已有双栈并发传输与流式请求期间状态查询的验收继续覆盖业务共存。
...
macOS 也能先跑共享业务合同，不必等到 QEMU。未显式给出时，宿主入口按
`scripts/resolve-ucode-runtime.sh` 依次尝试 `NETFLEET_MACOS_RUNTIME`、仓库内已构建的
...
覆盖服务写动作、组合确认以及跨页工厂复用与独立权限。`https_lifecycle_contract.uc`
以受控时钟与进程边界执行实际控制器，验证交互排空时限、包更新等待及失败后的状态恢复；
实际网络和进程退出仍由 OpenWrt 资格与设备回读验证。
...
比较报告同时保留基线与候选身份、采样次数、预热方式及并发操作；条件变化时说明不可比项。
少量 status 样本的改善只证明该条件下控制面读取变快，不外推为选优耗时、转发吞吐或长稳结论。
...
完整候选的 QEMU 资格与旧目标的部分插件升级是不同组合证据。服务接口或包依赖变化时，
在隔离环境覆盖最小支持旧组合到精确候选集合的更新，以及新旧插件保留共存时的实际调用。
包依赖模拟只验证声明约束；还需通过受影响的正式 caller 验证参数、返回值和失败恢复。
目标未改变的插件也属于最终组合，不能从验收身份中省略。
HTTPS 的保留包组合入口见[插件开发](plugins.md)：在合格基础上安装目标保留的精确签名
插件，再运行完整独立资格；由组合回执绑定实际文件，而不是放宽安装端的字节检查。
...
进程 CPU ticks 和 RSS；它不执行选优、启用或配置保存，不能证明这些工作流通过。
受影响流程应分别取得真实动作结果：选优覆盖主出口和从属出口，启用回读运行 owner，
配置保存完成合法输入的校验、应用与生效读取。浏览器能打开、字段可见、无效输入被拒绝，
只证明对应交互；发现合法配置仍被拒绝时保留该失败，不以只读健康代替保存验收。

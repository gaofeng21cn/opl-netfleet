...
依赖；不逐插件重复扫描依赖图。产品编译通过 GNU Make jobserver 默认并行四个任务；
资源有限或排查时传 `--jobs 1` 使用串行（允许 1–16），SDK 准备与清理仍先顺序完成，
全程保留一个 SDK 写入锁。这不是同时启动多个构建器。最终文件清单也在一个进程中计算 SHA-256，按字节序

...
“代理”菜单和菜单栏状态项按页面上报的状态重建：运行结论与本地监听地址、启动与停止、
按出口分组的当前路径与地区快捷切换、网络接管方式、运行模式、重新选优和刷新状态；图标随运行
状态变化。两者与页面按钮共用同一条串行动作路径，网络接管与运行模式仍由页面弹确认框，
状态未确认、未运行或操作进行中时对应项禁用而不猜状态。

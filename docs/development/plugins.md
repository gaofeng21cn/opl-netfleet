...
使用组件页“服务组合”，或调用 `plugins-system-get`、`plugins-system-validate <file>`、
`plugins-system-apply <file>` 完成组合更改。请求文件为 `0600` JSON envelope：
`{"request":{"revision":"读取的 revision","config":{},"confirm":true}}`，其中 `config`
是完整私有覆盖，含 schema、bindings、enabled 及所需 config/instances。校验返回依赖错误
和受影响插件；应用会交接已存在资源，失败恢复原配置。新安装插件的常驻资源仍由插件
显式生命周期入口启动，启用配置不隐式创建后台进程。

只操作本插件私有数据的服务 `actions` 可声明 `"lock":"plugin"`；未声明时使用网络锁。
插件锁在默认和具名实例之间共享，读共享、写排他，同时持有数据共享租约，使备份恢复
与组合事务可以排除写入。不得在这种动作中更改 DNS、路由、共享核心或调用网络写动作。
代码租约仍独立保留，包更新必须等待在途调用完成。SDK 生成的包要求内核至少 0.8.1。

OpenWrt 插件可备份的持久文件放在 `/etc/opl-netfleet/plugin-data/` 下，使用按插件与实例
区分的路径、私有权限和原子写入。标准备份包含该目录和私有组合，不包含插件代码；
恢复前先安装匹配 API 的插件包。备份数据锁覆盖宿主动作；插件自有后台写入应通过
生命周期暂停或自身快照机制保证一致性，不能把活动数据库文件直接当作可移植备份。
...
新增必需参数、必需字段或改变返回值语义都属于接口变更。修改提供者时须追踪真实调用者，
同步参数传递、结果处理和失败恢复，并使服务版本约束及软件包依赖表达实际兼容要求。
服务声明检查已声明的接口版本，包管理器检查已声明的包依赖；两者都不会自动发现仍沿用
旧声明的参数或语义变化。包能安装、服务能绑定，不等于业务调用兼容。

独立升级应验证最小支持的已安装组合与完整新组合，覆盖被保留的旧调用者或提供者。
需要配套升级的插件作为同一兼容集合交付；完整新组合通过隔离验收不能证明任意混装
都受支持。验证方法归[插件与发布](validation.md#插件与发布)，目标组合核对、回退材料
和安装步骤归[有限插件组合更新](../operations/canary-promotion.md#有限插件组合更新)。
...
检查隔离 origin 返回的实际协议，协议不符计入错误。比较报告保留失败请求、各轮波动、
绝对延时与 CPU 总量，不把短采样、未确认协议的历史负载或旁路状态解释为转发性能。
每个窗口同时检查内核接管、管理状态新鲜度、上传字节数和完整 SSE 事件数；任一窗口
旁路、超时、业务结果不完整或缺少采样，该场景不计算优化百分比。统计同时保留节流
累计秒数和进程峰值。隔离上游使用持久 HTTP/1.1 连接与明确响应边界，避免测试服务
主动关闭连接把重连成本误算为插件开销。
每轮使用相同的 100 条合成历史事件，隔离客户端每三十秒通过真实 ARP 流量维持地址
证据；这两项在关闭、空闲、负载和 UI 场景中保持一致，不注入地址或延长信任 TTL。
同机同负载的管理开销减半和空闲低于单核 5% 是优化目标，必须与实际结果分开报告；
目标未达到不能靠增加 CPU 配额、降低必要健康检查频率或延长租约达成。
...
不包括开发机生成器、测试服务或供其他平台下载的工具；不得用整仓关键词扫描代替。

在无 Python/Node 的独立 OpenWrt guest 完成安装、启停、业务调用、升级回退和卸载，
外部测试驱动留在测试主机。分别报告压缩包、逻辑文件和完整缺失依赖安装增量，
以及空闲和固定负载下的 CPU、RSS、子进程数与业务时延。进程配额不是实际使用量。
高频轮次不得重复启动解释器、展开无变化的完整状态或进行无关网络探测。

可选增强只撤销自己的新流量接管；基础后端保留其全局网络生命周期职责。插件停止、
卡死或卸载时必须验证基础路径恢复，不能把软件包独立安装当作故障隔离证明。

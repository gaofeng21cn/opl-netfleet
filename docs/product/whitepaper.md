...
协议兼容、实时运行面板等可独立安装、维护或退出的能力适合形成扩展模块。模块化的
价值是限定依赖、私有状态和故障影响。微内核保留发现、依赖、权限、生命周期、事件通信
和事务协调等组合机制；订阅、选路、配置编译、协议适配、诊断及恢复策略都可以成为插件。
系统配置明确必需插件及其依赖，模块各自拥有状态和职责；基础联网与恢复不依赖某个
任意扩展的持续健康。Cordis 的服务组合与依赖注入理念是这一方向的参考。
模块声明连接有限动作和已有 UI 插槽，当前边界见[模块与扩展](../architecture/extensions.md)。
Cordis 等组合框架只有在真实管理端需求证明收益后才采用，不成为 OpenWrt 数据面依赖。

扩展体系支持按需接入并持续完善热加载：用户应能加载新模块、更新实现和退出模块，让协议
适配、诊断及其他网络能力独立扩展。模块来源、发现与分发也可以进一步发展为插件市场。
热加载的设计重点是接口兼容、依赖解析、状态交接和连接排空，使模块变化与基础选路、
恢复能力解耦。这里表达长期设计目标；当前接入方式由上述模块合同维护。
...
这五类能力构成产品的网络管理核心，机场无关策略和设备端结构化配置为其提供共同基础。
原生管理、后端适配和可选扩展围绕同一核心演进；Zashboard 提供实时观察体验，模块化
为进一步增加独立能力提供空间。具体实现与交付范围分别由[当前架构](../architecture/overview.md)
和[README](../../README.md)维护。

...
用户选择完整 Zashboard 的主要价值。所选后端已经拥有静态资源与 controller 配置，导航无需再建立一套运行事实。

## 决定

Zashboard 是独立、完整的 Mihomo 实时运行面，不嵌入 NetFleet 内容区。NetFleet 负责统一产品
导航、就绪判断和打开动作；资源与连接信息从所选后端取得。Zashboard 内的 selector
切换和连接管理属于 Mihomo 当前运行态；NetFleet 仍唯一拥有持久 policy、订阅编排、
服务启停和恢复事务。两种后端使用同一 NetFleet 打开入口；当前调用与凭据边界只在
[公开接口](../architecture/interfaces.md)维护。
...
- 为导航另建静态资源副本、controller 或同源代理；
- 把 Zashboard 的即时运行态操作当成 NetFleet 持久配置；
- 为非 Mihomo 后端伪装 Mihomo API。

## 重审条件

当上游提供能保留独立完整页面体验的替代认证接口时，重新评估 URL 凭据方式；只有

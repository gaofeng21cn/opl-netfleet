...
先区分已发布包的设备更新和新版本开发交付。若只是把已验证的插件装到另一设备，直接
使用下列 Feed 入口，不重新构建或运行 QEMU；若功能尚未发布，先按
[构建与候选 SOP](../development/delivery.md#构建前预检与候选复用)完成一次候选及所需资格，
所有目标消费同一资产。不能把本机有 SDK 目录等同于宿主能执行该 SDK。
...
现有回退归档；不为每个 RPC 新建隧道。网络失败先读取保留的 journal，不重新发起安装。
Home、Lab 等多个目标分别授权和建立基线，但不重复构建、下载相同的本机候选或重新资格。
目标名和连接参数仍只从私有上下文取得。
...
独立插件更新沿用“基础组合 owner、插件 owner、设备 owner”三层责任。基础组合 owner 先提供与目标实际安装组合匹配的最新 qualification receipt；插件 owner 只提供签名插件包和插件资格，不重新打包或替换基础 NetFleet。设备 owner 在单一 operator/deploy 锁内执行 dry-run、排空、安装和回读；前置身份、依赖或网关检查失败都在 APK 写入前旁路并结束，不停止基础 NetFleet。独立插件事务默认只观察 10 秒，安装等待最多 300 轮，成功立即结束等待；维护期间使用保留事务代码读取终态，超时按同一 journal 对账，不重复安装。十秒仅是完成后的默认观察窗口，不是安装超时或业务恢复保证。
...
设备更新只消费冻结的签名包与已有资格，不在每个目标重新构建或运行 QEMU。
有限插件安装器先读取已安装版本，过滤同版本包；全部无变化时返回 `no_change`，
不传输归档、不准备设备事务，也不重新寻找回退包。这只表示无需版本更新，不证明
设备文件没有损坏；修复安装仍需独立诊断与明确的重装操作。
宿主回执的 `timings` 分别记录候选校验、设备版本读取、准备、传输、事务与观察及总耗时；
设备 journal 的阶段时间用于定位安装过程。公网获取、开发构建与资格验证耗时单独报告，
不能把保护性观察时间当成安装时间，也不能把秒级安装当成整个开发交付已优化完成。
有效输入下无变化检查以三秒以内为目标，包已就绪的小型无常驻插件以三十秒以内为目标，
独立进程插件在没有健康长连接时以六十秒以内为目标；目标不是强杀连接的截止时间。

...
“更新最新版”默认指目标已配置、签名可信且已通过发布资格的 Feed 最新版。直接使用下列
Feed 入口；源码比 Release 新，不自动授权开发、构建或发布新版本。只有用户明确要求
交付尚未发布的功能时，才进入[构建与候选 SOP](../development/delivery.md#构建前预检与候选复用)。
流程优化、文档改动、其他任务的遗留和浏览器登录不加入普通设备更新的前置条件。

日常操作先复用本次已有的设备与签名索引快照；索引过期或尚未检查时，通过组件原有
检查入口刷新一次。用正式更新器取得计划并执行一次事务，消费其最终回读。目标已经
满足请求且无故障证据时，以 `no_change` 结束；不要为版本号整齐重装、降级或继续搜索
未发布源码。一次事务已覆盖的依赖不再逐包更新。多个目标各自执行设备事务，
共用已发布资产；不逐设备构建、跑 QEMU 或发布。目标名称只从私有上下文取得。

每次先读取目标的组件目录和实际版本，只更新有变化的插件。APK 负责依赖求解，维护者
核对生命周期影响；共享模型或平台插件可能影响核心依赖链，不能仅凭“只更新插件”承诺
核心不会重启。没有共享运行依赖的页面插件才可以把核心 PID 不变作为硬验收。

操作按“预览 → 一次事务 → 回读”执行。保留同一 SSH 控制连接，复用有效的签名索引与
现有回退归档；不为每个 RPC 新建隧道。常规 Feed 更新省略 `--rollback-dir`，优先使用设备原安装事务
保存的签名归档；仅在确实缺失时提供外部回退包。同版本归档也可能来自不同构建，
不得覆盖设备已留存的原始归档。网络失败先读取保留的 journal，不重新发起安装。
多个目标分别授权和建立基线，但不重复构建、下载相同的本机候选或重新资格。
目标名和连接参数仍只从私有上下文取得。

Feed 模式的宿主回执记录版本读取、计划、准备、传输、事务加观察和总耗时；设备结果中的
`device_timings` 再拆分准备、事务和观察。采用单调时钟，避免时钟校准影响差值。开发构建、
资格、公网下载和设备事务分别汇报；“约 90 秒”只能作为待验证的目标，不能把构建及资格
耗时隐去，也不能为满足目标缩短正在正常排空的连接。另报从开始处理更新请求到最后
目标回读结束的端到端墙钟耗时；候选返工、重复验证和人工编排等待也计入这个总量，
不能用设备事务的一分钟替代整个交付耗时。多个阶段并行时不把子耗时简单相加。
已发布版本的常规更新，其构建、QEMU 和发布次数应全部为零。
...
相同版本直接返回 `no_change`。`--dry-run` 只预览当前签名索引，不启动事务。无变化时不传输归档、
不准备设备事务，也不重新寻找回退包；这不证明设备文件未损坏，修复安装仍须先诊断。
独立插件事务默认在安装完成后观察十秒，健康长连接按实际排空情况处理，不能为满足
时限强杀。维护期间使用保留的事务代码读取终态，不依赖正在更新的插件入口。

| 日常操作 | 设备执行目标（不含公网下载、构建与发布资格） |
| --- | --- |
| 有效索引下确认无变化 | 三秒以内，不改变运行状态 |
| 已备包的小型页面或无常驻插件 | 三十秒以内完成安装和回读 |
| 已备包的独立进程插件，无健康长连接 | 六十秒以内 |

超过目标先读取阶段计时和同一事务状态；目标不是超时回滚条件。
...
`--plugin` 可重复，也可显式选定 `luci-app-netfleet`；目录中其他包不进入安装集合，旧版归档按目标当前版本精确提供。
...
| 独立插件维护 | 先查看实际包变化再确认；安装不等于启用；禁用且无依赖后卸载；升级失败回退，基础网络与私有配置保留 |
...
判断网站体验时先查看实际连接、命中规则及出口链，再区分订阅配额、节点探测与站点请求
失败。保护探针成功只证明所测目标；没有该网站的请求证据时，不宣布解锁、吞吐或长连接
正常。对照应使用同一客户端、同一站点和相近时段，每次只改变一个网络选项；订阅来源
未变化且缓存有效时，不用反复更新订阅代替业务诊断。

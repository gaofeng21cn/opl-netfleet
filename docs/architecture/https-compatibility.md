...
续租拒绝或控制链异常同时重置连续健康计时，恢复后仍须经过三十秒验证；
基础网关拒绝准入不累计插件故障，控制链超时或失联只在此前已接管时累计一次。

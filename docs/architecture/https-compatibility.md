...
网关准入通过原生 ubus 读取进程状态，并复用归属链存在证据。LAN 接入集合保持独立读取，避免 nft 合并读取缓存遗漏集合元素；同一调用中的核心进程身份复用，跨调用不缓存。私有表正常存在时直接读取，失败才查询存在性以区分缺表和读取错误。租约写入后仍单独从内核回读实际集合与到期时间；同一 nft 写批次中的 list 输出来自提交前缓存，不能冒充写后验证。读取失败不能解释为成功旁路。

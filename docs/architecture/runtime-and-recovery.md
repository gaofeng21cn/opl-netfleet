...
调度插件只向宿主返回失联起点、选择期限、上一轮就绪状态和等待时间；代码在下一轮重新加载，这些定时状态保留在宿主内存。订阅刷新期限每轮从 SubscriptionOwner 的持久记录计算，不保存在宿主内存中。插件不修改后端 respawn 参数、不捕获每个进程事件、不轮询远端业务 URL；平台就绪检查与业务探针分别解释，业务验证仅发生在恢复事务内。自动恢复意图与运行事实不依赖该内存状态。

...
2. 保留 Policy Source 的规则顺序、DNS 和未绑定组；迁移 Profile 中已有的 DNS 逐字保留，内置 bundle 自带不含秘密的可用 DNS 基线：普通域名和明确直连出口优先使用 Mihomo 的 `dhcp://wan`，继承 WAN DHCP 下发的光猫/运营商本地 DNS 以降低国内解析延迟；`geolocation-non-cn` 规则集使用 Cloudflare/Google 加密 DoH，代理节点域名只使用加密 TLS 上游，未命中规则或本地主解析返回非中国结果时由加密境外 fallback 配合 GeoIP-CN、保留地址和常见境外域名过滤兜底；`respect-rules` 保持关闭，避免新安装时 DNS 上游反向依赖尚未就绪的代理。Nikki 继续拥有 DNS 运行生命周期，全局 mixin 只按字段叠加 target/provider 特例，不要求每个 private Instance 复制通用基线。target-local `routing_rules` 在开头连续的管理链路直连规则之后、其他分类规则之前投影为 Mihomo 规则，目标由 capability ID 解析为可见 selector；它们只存在于生成 Profile，不进入 Nikki 全局 mixin 或 Recovery Profile。随后把每个 `entry` 的规则目标及组引用改写到 capability 可见 selector，删除旧入口及仅由该入口可达、现已 caller-zero 的旧 Auto/地区闭包；不依赖 `hidden` 维持用户界面正确性；
...
`/var/lib/opl-netfleet/events.json` 是固定上限的持久化 owner 事件，不是 operation history 或选择输入。它记录 one-shot owner 已实际完成的 enable、select、disable 和 subscription refresh；refresh 事件只保存执行时间、聚合结果、机场总数/变化数/失败数、是否重载、调用来源，以及每个稳定 section 的 `result`/`digest`，不保存 URL、token、节点、cache 内容或完整配置。写入失败不改变数据面结果。调用来源只允许 owner 已知的 `luci|cli|deployer|supervisor`，未知入口如实记录 `unknown`，不能据进程或时间猜测。Mihomo 自主 health-check/fallback 继续写 Nikki 管理的 core log，NetFleet 日志页只读展示其中与 `NETFLEET-` 相关的最近行，并明确受 Nikki 自身清理策略约束；NetFleet 不为捕捉每次叶子变化增加常驻监听器。
...
- refresh 仍是 `main.uc` 的唯一 writer：枚举 policy 中 enabled provider 的稳定 Nikki section，逐个调用 Nikki 官方 updater。每个调用前保存该 section 的 LKG cache；上游不可用时不更新；下载失败、空内容或 YAML 无法解析为非空 `proxies` 时立即恢复该机场旧 cache 并继续其他机场；一个机场失败不得擦除其他已验证 cache。全部摘要不变时不 compile、不 restart、不触碰 selector。cache 有变化且 NetFleet active 时，owner 保存旧 artifact/manifest 和全部可见 selector，重新 compile、restart、恢复用户模式并立即执行同一个 automatic 轮次和 protected probes；任何 compile、owner readback、选择或探针失败都恢复全部更新前 cache、artifact/manifest 与 NetFleet runtime。NetFleet inactive 时只更新 Nikki cache，不擅自 restart 或接管原生数据面。status 通过同一 owner 投影每个 enabled subscription 的 `section`/`ref`、`display_name`、`cache_present`、`cache_sha256`、`quota`、`last_attempt`、`last_success` 和 `last_result`；refresh 事件保留每 section 的 `result`/`digest`，不含 URL/token/正文。

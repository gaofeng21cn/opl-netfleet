...
- refresh 仍是 `main.uc` 的唯一 writer：枚举 policy 中 enabled provider 的稳定 Nikki section，逐个调用 Nikki 官方 updater。每个调用前保存该 section 的 LKG cache；上游不可用时不更新；下载失败、空内容或 YAML 无法解析为非空 `proxies` 时立即恢复该机场旧 cache 并继续其他机场；一个机场失败不得擦除其他已验证 cache。全部摘要不变时不 compile、不 restart、不触碰 selector。cache 有变化且 NetFleet active 时，owner 保存旧 artifact/manifest 和全部可见 selector，重新 compile、restart、恢复用户模式并立即执行同一个 automatic 轮次和 protected probes；任何 compile、owner readback、选择或探针失败都恢复全部更新前 cache、artifact/manifest 与 NetFleet runtime。NetFleet inactive 时只更新 Nikki cache，不擅自 restart 或接管原生数据面。status 通过同一 owner 投影每个 enabled subscription 的 section/ref、显示名、cache digest、quota/expiry 和最近 refresh 结果；

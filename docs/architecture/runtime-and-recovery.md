...
2. 保留 Policy Source 的规则顺序、DNS 和未绑定组；迁移 Profile 中已有的 DNS 逐字保留，内置 bundle 自带不含秘密的可用 DNS 基线：国内加密 DoH 承担默认与代理节点域名解析，`geolocation-non-cn` 规则集使用 Cloudflare/Google 加密 DoH，未命中规则或主解析返回非中国结果时由加密境外 fallback 配合 GeoIP-CN、保留地址和常见境外域名过滤兜底；`respect-rules` 保持关闭，避免新安装时 DNS 上游反向依赖尚未就绪的代理。Nikki 继续拥有 DNS 运行生命周期，全局 mixin 只按字段叠加 target/provider 特例，不要求每个 private Instance 复制通用基线。target-local `routing_rules` 在开头连续的管理链路直连规则之后、其他分类规则之前投影为 Mihomo 规则，目标由 capability ID 解析为可见 selector；它们只存在于生成 Profile，不进入 Nikki 全局 mixin 或 Recovery Profile。随后把每个 `entry` 的规则目标及组引用改写到 capability 可见 selector，删除旧入口及仅由该入口可达、现已 caller-zero 的旧 Auto/地区闭包；不依赖 `hidden` 维持用户界面正确性；

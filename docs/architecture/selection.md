...
选路提交前，activation 按依赖顺序对已选 preferred selector 使用 path probe、对内层 proxy path 使用 guard probe，刷新 Mihomo 按 URL 独立保存的健康状态，再回读完整优选路径。测速 URL 的成功不能替代这两个业务 URL 的健康事实。验证只经过所选链，不遍历备用分支，不固定 fallback，也不把业务探针延迟写入排序；任一层验证失败保留具体错误并走原事务恢复。

...
速度、Mihomo 候选健康、配额和关键业务保护是不同事实。延迟与配额由独立适配器标准化，候选健康来自同轮 Mihomo owner readback 中 `extra[checks.latency.url]` 的独立健康状态；组和 manifest 绑定的真实节点都必须通过该测速 URL，不能使用由其他 URL 测试覆盖的全局 `alive`。protected probes 既决定一次 enable/select 是否可提交，也为已生成的两层 Mihomo fallback 提供独立运行期健康目标，但其耗时和结果不进入 comparator。这些事实不能互相代换。

### 测量事实

| 事实 | 目标与方法 | 标准化结果 | 明确禁止 |
| --- | --- | --- | --- |
| 延迟 | enable、显式 `select auto` 或 supervisor 到期轮次先记录各 Provider/地区 URLTest 的历史尾部，再按 `checks.provider_healthcheck_timeout_ms` 并发触发每个 provider 一次 Mihomo 原生 health-check，并对未覆盖的候选集合做 capability group delay，同源同叶子的后续出口复用本轮快照；selector 结果未覆盖的组，只接受本轮后历史时间戳确实变化、delay 大于 0 且组和叶子仍健康的结果。单个 provider 失败只使其候选失去本轮证据 | 每个返回项为 `status=ok,delay_ms=<正整数>`，缺失项为 `unavailable`；最终叶子必须在所选组的本轮成功结果中 | 用单节点 delay timeout 提前终止整机场批量检查、supervisor 自建第二扫描算法、旧 URLTest 历史补位、逐候选 shell 串行请求、直接 ping 共享入口、业务探针耗时或历史平均值；不带期望状态接收任意响应 |
| 候选健康 | 同轮读取 Mihomo Provider/地区组和真实叶子的测速 URL 独立健康记录与身份 | `available=true|false` 和稳定叶子身份；缺失、失败及无本轮 delay 分别记录原因 | 使用全局 alive 覆盖指定测速 URL 的结果；把业务 URL 耗时、入口 ping 或旧历史当作候选健康 |
...
5. **能力组合**：每个 capability 先按自己的授权范围和门槛过滤候选。automatic capability 形成一个无环依赖图：唯一根能力先选择；跟随能力在根能力所选节点对自身仍合格时复用同一节点及同一份延迟；该节点不合格时选择自身同轮最快合格候选，不因旧地区的切换门槛滞留。依赖来自 `prefer_region_from`，engine 不按 `standard`、AI、地区或机场名称分支。
...
Provider/地区和 Provider 代理组在过滤后为空时使用 `REJECT` 作为不可用终端，
不能让空组经 `DIRECT` 得到成功延迟并参与上层地区或机场竞速。地区外层与自动路径的
直连护栏继续保留：全部代理不可用时才进入直连；手动指定地区不能把该退路提交为成功。
手动事务先测量指定地区各机场层的候选，再按该地区的 path probe 刷新各层当前路径，
最后回读地区 fallback 的真实叶子；不能仅测外层 fallback 就假定嵌套组已收敛。
共享地区出口的 manifest 同时携带该地区的候选身份，跟随能力按共享链回读 provider、
地区和真实叶子，不能拿自身 automatic 组名匹配上游地区路径。

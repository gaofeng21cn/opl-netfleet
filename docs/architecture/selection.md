...
2. **地区代表**：每个授权地区取本轮 `available=true` 且 `latency=ok` 候选中最小 `delay_ms` 的叶子；同地区代表的同速比较复用下述 quota 与稳定身份顺序；跨地区比较在同速时还比较稳定 region ID，具体确定顺序由 `core/selector.uc` 的 `best_region` 定义。

...
| 配额 | 读取当前后端 subscription metadata；不发额外业务请求 | `available`、`exhausted` 或 `unknown`，可带 `remaining_bytes`；手工月重置日仅供 UI 参考，见[显示合同](interfaces.md#luci-显示层合同) | 把 ping 不通判为耗尽，把 unknown 判为耗尽，按参考日期清零用量，或假定买断一定无限量 |

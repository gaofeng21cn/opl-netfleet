...
机场和地区表优先呈现“最近一次测速”，只读取根能力最近一轮的 `measurement`，持续显示采样时间及成功数量，存在异常时也不隐藏成功。历史测量归机场详情或地区行内展开，不补入最近一次结果。流量耗尽是主要限制，测速断点独立保留；每项明细列出机场、地区及具体结果，说明计数单位是机场在一个地区的候选线路，不是节点数。没有有效结果时显示“未取得有效测速”，不能断言整个地区或业务目标不可用。旧记录没有细分原因时明确说明，未知错误不编造根因。测速结果不是业务保护探针结果，也不代表实时状态；手动保持期间最后一次采样时间不刷新。

本文定义 owner 状态如何解释为用户可见信息；不定义 RPC、候选排序或视觉布局。
组件页尚未取得首次读取结果时显示“正在读取已安装组件”，读取失败显示实际错误并允许重试；
空快照不能用来判断设备缺少组件管理接口，也不能建议用户因此升级。
...
机场详情和地区行内展开都显示 evidence 中的“最近最优”“平均最优”、有效样本数与最近有效测量时间。没有有效样本时合并延迟空态为“暂无有效测量”，仅有一次时显示“仅 1 次测量”，达到两次后才显示真实平均，即使与最近值相同也不改写。机场值是一轮内该机场全部合格地区候选的最小有效 delay，地区值是一轮内该地区全部合格机场候选的最小有效 delay。历史保留范围来自 manifest 的当前候选目录，不来自某轮成功解析的运行候选；临时失效或本轮未取得叶子不清空历史、不增加样本，也不把旧测量时间刷新为本轮时间。删除目录对象时裁剪，改变延迟测量口径时重置。UI 明确区分库存健康和有效测量历史，旧延迟不冒充本轮成功。

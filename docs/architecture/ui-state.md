...
机场和地区表优先呈现“最近一次测速”，只读取根能力最近一轮的 `measurement`，持续显示采样时间及成功数量，存在异常时也不隐藏成功。历史测量归机场详情或地区行内展开，不补入最近一次结果。每项明细先显示 SubscriptionOwner 当前配额记录和订阅更新时间；耗尽时主提示为“流量已耗尽，不参与选优”。配额来自最近一次订阅更新，不随测速刷新，界面必须明确这一新鲜度边界。该次测速的成功或断点保留为独立记录；测速时耗尽但当前记录已恢复时明确标为“该次测速时流量已耗尽”，不沿用为当前限制。每项明细列出机场、地区及具体结果，说明计数单位是机场在一个地区的候选线路，不是节点数。没有有效结果时显示“未取得有效测速”，不能断言整个地区或业务目标不可用。旧记录没有细分原因时明确说明，未知错误不编造根因。测速结果不是业务保护探针结果，也不代表实时状态；手动保持期间最后一次采样时间不刷新。
...
地区目录与当前地区规划是两层对象。policy 的 `regions` 与 `provider_regions` 只定义稳定地区 ID、显示名、Provider filter 映射和 capability 许可；status 完整投影目录供 owner 关联运行状态和历史，目录项本身不代表当前必须可用。当前地区规划只包含同一次设备状态中 `available_count > 0` 且 `available_provider_count > 0` 的地区：它是首页当前可用地区数量、最近最优和平均最优的统计边界。地区浏览页呈现完整已配置目录，当前规划作为健康状态投影；未接管时显示未测量，不把目录隐藏。浏览目录不授予操作资格，切换仍要求 capability 当次返回的授权地区和运行前置条件。`available_count` 表示真实候选路径数量；`available_node_count` 只是后端库存诊断，可能未知，不能作为地区可用性的门槛。机场和地区的实时可用数只有在 NetFleet 已接管、生成配置存在且 Mihomo 控制面可读时才具有故障语义；NetFleet 未接管或控制面不可读时，UI 必须显示“未测量”，不得把 status 中用于占位的零值解释为机场或地区下线。没有真实可用路径的目录项不进入当前规划、不作为首页当前可用地区的分母；明确标为“已配置地区”的目录计数可包含它，也不触发“不可用地区”警告；只有当前正在使用的地区失去真实路径时才作为运行异常提示。不能用“至少两个节点”或“至少两个机场”等数量门槛排除合法的小众地区，一条真实可用的 Provider/节点路径即可进入当前规划。

机场表的“可用地区”是去重后的 provider-region 数量；地区表用“机场健康记录”“节点健康记录”呈现当前核心库存的健康数/总数，不能将通用 `alive` 标记命名为本轮测速成功数或当前选优资格。机场健康数是去重后的 region-provider 数量，要求其候选组和至少一个真实节点的通用健康记录成功。机场的“节点”是 manifest 绑定 source 在 `/providers/proxies` 中的 Mihomo 已加载库存，按节点名去重并使用 `alive` 统计健康数，不因地区识别或 capability 许可排除小众节点；当 SubscriptionOwner 读取的原始订阅条目数与已加载数不同，机场同一单元格补充“订阅 N 条”，不能把未被 Mihomo 接受的条目计作可用节点。地区的“节点”以 `/proxies` 当前 group 的成员关系为边界，并用同一 source 补充 file-provider 叶子身份和健康状态。通用健康与本轮候选测速的目标、采样时间和计数单位可能不同，UI 在表格旁说明区别；配额耗尽仍可有核心健康记录，是否参与选优以配额及本轮候选资格为准。按 Mihomo 类型确认的直连、拒绝等控制面终端是已知的非节点，排除后不影响其他候选组计数；不得按节点显示名猜测类型。缺失成员列表或不能解析的嵌套组仍是未知，显示“节点清单暂不可读”，不能伪造零值。没有真实可用叶子的地区不能凭历史 evidence 占位。
...
机场页从 SubscriptionOwner 的独立持久状态显示“最近全部更新”（最近一次全部订阅成功接受的完成时间，也是每日调度基准）、“最近尝试”、最近结果和下一次全量更新计划。失败保留上次成功时间；无可信成功记录显示“暂无成功记录”，无尝试记录显示“暂无执行记录”；逾期或没有全量基准显示“待更新”。旧事件只在尚无独立状态时用于恢复可证实的逐来源结果，不推断旧事件属于全量更新；首次刷新写入独立状态后不再依赖事件历史。

机场详情把 SubscriptionOwner 的 `last_success` 标为“订阅更新时间”，把 `last_attempt` 标为“最近尝试”；没有独立记录或可迁移的旧刷新记录时，“最近尝试”显示“尚未执行”，“订阅更新时间”显示当前后端订阅缓存的实际修改时间，缓存也不存在才显示“尚未执行”，不得拿最后测量或 cache digest 猜更新时间。订阅制机场从同一次 target-local 后端 metadata 读取 `expire` 并投影为 quota `expires_at`；缺失显示“机场未返回到期时间”，买断制显示“不限时间”，不得从节点名称或订阅 URL 猜测。地区默认按本轮有效 delay 升序、可用节点数降序、可用机场数降序及显示名排列；没有本轮有效值的项放在末尾，历史延迟不参与本轮排名。“当前使用”只作标记，不改变位置；历史最近与平均最优仍可显式选择排序。机场表继续按当前选中项、可用地区数、最近最优 delay 和显示名排序。两者都只是展示顺序，不改变运行时 comparator 或 selector。

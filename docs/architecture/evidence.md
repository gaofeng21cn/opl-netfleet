...
候选结果覆盖 manifest 中的全部候选组；一项对应根能力的一个机场/地区候选线路，不是一个节点。`measurement_reason` 独立记录测速或节点验证断点，`quota_state` 保存同轮配额事实，`reason` 在配额耗尽时优先报告资格限制，否则报告测量断点。成功测速仍可与配额耗尽同时存在，不得把耗尽当作连接失败的已证原因。节点验证区分候选组未加载、没有选中节点、订阅节点清单缺失、节点不属于订阅或身份重复、测速健康明确失败及健康记录缺失；没有底层错误证据时不猜测 DNS、TLS 或超时原因。

机场和地区的 `measurement` 投影根能力最近一轮的采样时间、有效测速最小延迟、成功数、主要限制计数及有界 `entries` 明细。每条明细只包含机场和地区稳定 ID、成功标记、延迟、同轮配额状态、主要原因和测量原因，不暴露节点、内部组名或完整配置。读取已有记录时使用其保存的 `quota_state` 优先呈现耗尽；缺少细分原因则明确旧记录未保留细节，不从当前状态追溯猜测历史原因。历史聚合不能补入本轮值。无证据时为 `null`，没有有效测量时 `best_delay_ms` 为 `null`。测速成功数不等于通过地区授权、故障层级和业务保护的最终入选数。

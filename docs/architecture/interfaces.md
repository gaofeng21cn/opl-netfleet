...
“插件”是宿主提供的统一目录，按插件及实例分组列出所有可用页面；进入后保留返回目录
及同插件页面切换。产品插件仍负责其业务页，目录只负责发现与导航，不复制安装、启停、
卸载或配置动作。“组件与更新”保留配置直达和生命周期管理。禁用、卸载或资源不可用后
移除对应导航，当前页面失效时回到默认入口；清单刷新保留仍有效的页面及实例身份。

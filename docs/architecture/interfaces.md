...
`events` 默认只读取持久事件；仅核心日志消费者显式指定 `include_logs=true`
（命令行为 `events logs`）时读取有界核心日志。普通页面仅在展示事件时请求 events。
同页重复状态读取共享在途请求；配置写入后的强制回读必须等待旧读取结束后重新读取，
不能被旧请求合并。卸载页面不再消费异步结果。Dashboard 连接信息只在用户打开时读取，
只保留在本次页面内存，不进入展示缓存。
...
组件更新区分替换前准备、正在替换、已取消、已延后和恢复结果。取消请求不等于取消完成，
收起提示也不等于取消；只有 owner 确认恢复成功才显示旧版与运行状态已保留。
用户提示解释原因、当前影响和下一步，原始错误码折叠为技术详情。
配置页面分别呈现本地草稿、设备保存值和运行应用状态；不得从草稿与保存值相同推断已生效。

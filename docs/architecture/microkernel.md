...
普通命令发现只读取清单；调度、命令执行及具名 `plugin-read` 按目标与实际服务依赖闭包检查插件文件，未使用插件的
资源目录不参与深度扫描。`plugin-read` 先读取声明选择锁，取得锁后重新检查目标及锁声明，
服务依赖和进程入口仍须在代码租约内核验当前字节。完整组件目录、组合变更与包生命周期仍检查完整插件集合。
清单仅用于定位，不能授权执行；选中命令在完整检查后重新核对声明，执行仍须通过代码租约。
...
平台文档解析可在插件自己的周期状态中保留有界缓存；每次读取仍核对当前完整源字节，
变化、读取失败或格式错误立即使对应缓存失效。返回独立副本，调用者不能污染后续读取。
缓存不保存运行健康结论，不延长故障检测或接管许可，也不成为配置写入授权。
...
Device identity。全功能安装首次安装 HTTPS 管理插件时，通过通用插件入口加载管理能力；已有插件的加载选择保持不变。安装范围与功能开关独立，不修改已有功能配置，不自动开启 HTTPS 转发。

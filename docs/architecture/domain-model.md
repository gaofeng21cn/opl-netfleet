...
| Owner | 唯一责任 |
| --- | --- |
| Policy Source | 提供流量分类、首条命中规则顺序、原始组名、DNS 和未绑定组行为；内置 bundle 使用锁定 MRS 做业务分类，并提供 WAN DHCP 本地主解析、境外规则集加密解析、代理节点加密解析以及 GeoIP-CN 过滤的境外 fallback 的无秘密 DNS 基线；`kind=profile` 继续逐字保留原 Profile 规则和 DNS；只作为 compiler 输入 |

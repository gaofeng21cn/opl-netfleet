...
const services = {
 "mihomo.controller": {
  ...
  test_group_path: (secret, group, checks) => {
   push(calls, { group, url: checks.latency.url, timeout: checks.latency.timeout_ms });
...
const policy = { checks: { latency: { url: "https://speed.test/204", timeout_ms: 2000 } },
 ...
 probes: [{ id: "path", url: "https://business.test/path", expected_status: 200 },
...
check(length(calls) == 3 && calls[0].group == "candidate" && calls[1].group == "preferred" &&
 calls[2].group == "inner" && calls[1].timeout == 15000, "probe only the chosen chain in dependency order with health timeout");
...
check(failed.error == "selected_business_path_probe_failed" && stale && length(calls) == 5,

...
export function DesktopTools({ snapshot, disabled, client, run, section }: { snapshot: DesktopSnapshot; disabled: boolean; client: DesktopNetFleetClient; run: RunAction; section: 'connections' | 'logs' | 'backup' }) {
  ...
  return <>
    ...
    {section === 'logs' && <section className="nf-log-section"><div className="nf-section-heading"><h2>Mihomo 日志</h2><button className="nf-button-secondary" disabled={disabled} onClick={() => void run('读取日志', async () => { setLogs((await client.logs()).text); })}>读取日志</button></div><pre>{logs === null ? '尚未读取日志。' : logs || '当前没有日志。'}</pre></section>}
    {section === 'backup' && <section className="nf-config-section"><div className="nf-config-section-heading"><h2>配置备份</h2><p>包含私有配置与订阅，请保存到可信位置。恢复前必须停止当前代理。</p></div>{error && <p className="nf-inline-warning" role="alert">{error}</p>}<fieldset disabled={disabled} className="nf-desktop-fieldset"><div className="nf-desktop-actions"><button className="nf-button-secondary" disabled={!snapshot.runtime.configured} onClick={() => void run('导出备份', exportBackup)}>导出备份</button><DesktopFilePicker label="选择备份…" accept=".json,application/json" disabled={disabled || snapshot.runtime.running} onFile={file => { void file.text().then(text => { setRestore(parseObject(text)); setError(null); }).catch(reason => setError(failure(reason))); }} /></div></fieldset></section>}

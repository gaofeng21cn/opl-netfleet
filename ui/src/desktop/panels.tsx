...
export function RuntimeControls({ snapshot, disabled, client, run }: { snapshot: DesktopSnapshot; disabled: boolean; client: DesktopNetFleetClient; run: RunAction }) {
  ...
  return <>
    <section className="nf-config-section nf-desktop-runtime">
      <div className="nf-section-heading"><div><h2>代理核心</h2><p>负责节点连接与自动选优，独立于下方的本机流量接入。</p></div><span className={`nf-desktop-state ${runtime.running ? 'is-running' : ''}`}>{runtime.running ? '运行中' : '已停止'}</span></div>
      <div className="nf-desktop-runtime-summary"><div><strong>{runtime.mode === 'unconfirmed' ? '运行状态待确认' : modeNames[runtime.mode]}</strong><p>{runtime.running ? `本机代理地址 127.0.0.1:${runtime.ports.mixed} · HTTP / SOCKS5` : snapshot.runtime.configured ? '配置已准备，可启动代理进行连接测试。' : '先添加机场订阅，自动准备配置。'}</p></div><div className="nf-desktop-actions"><button className="nf-button-primary" disabled={disabled || !runtime.configured || connected} onClick={() => void run('启动 NetFleet', () => client.enable())}>{connected ? 'NetFleet 已运行' : '启动 NetFleet'}</button><button className="nf-button-secondary" disabled={disabled || (!runtime.running && runtime.clean)} onClick={() => void run('停止代理', () => client.action('mode', { mode: 'direct' }))}>停止代理</button></div></div>
      <details className="nf-desktop-advanced"><summary>原生配置模式</summary><p>使用导入配置自身的代理规则，暂停 NetFleet 自动选优。</p><button className="nf-button-secondary" disabled={disabled || !runtime.configured || runtime.mode === 'mihomo'} onClick={() => void run('使用原生配置', () => client.action('mode', { mode: 'mihomo', authorize: true }))}>使用原生配置</button></details>
    </section>
    <section className="nf-config-section" aria-label="本机流量接入">
      <div className="nf-section-heading"><div><h2>本机流量接入</h2><p>决定哪些应用使用代理。更换方式需要点击应用。</p></div><span className="nf-desktop-state">{networkState}</span></div>
      <fieldset disabled={disabled} className="nf-desktop-fieldset"><legend className="nf-visually-hidden">选择本机流量接入方式</legend><div className="nf-desktop-network-options">{(Object.keys(networkNames) as Array<keyof typeof networkNames>).map(value => <label className={network === value ? 'is-selected' : ''} key={value}><input type="radio" name="network-mode" value={value} checked={network === value} onChange={() => setNetwork(value)} /><span><strong>{value === 'explicit' ? '仅显式代理' : networkNames[value]}</strong><small>{descriptions[value]}</small></span></label>)}</div><div className="nf-desktop-network-footer"><p>{network === 'explicit' ? '适合订阅、节点和代理连通性测试。' : snapshot.network.helper === 'needs-install' ? '首次使用需要安装网络组件并由 macOS 管理员授权。' : !snapshot.network.authorized ? '网络组件已安装，应用时仍需建立授权会话。' : '网络授权已就绪。'}{network !== runtime.networkMode ? ' 当前选择尚未应用。' : runtime.networkMode !== 'explicit' && !runtime.running ? ' 启动核心后才会接入流量。' : ''}</p><button className="nf-button-secondary" disabled={network === runtime.networkMode} onClick={() => { if (network === 'explicit') void applyNetwork(); else setConfirmNetwork(true); }}>应用接入方式</button></div></fieldset>
    </section>
...
export function SubscriptionManager({ snapshot, disabled, client, run }: { snapshot: DesktopSnapshot; disabled: boolean; client: DesktopNetFleetClient; run: RunAction }) {
  ...
  return <section className="nf-config-section" aria-label="管理订阅">
    <div className="nf-section-heading"><div><h2>机场订阅</h2><p>添加后自动下载、校验并编译。准备完成后再到概览启动代理。</p></div><button className="nf-button-secondary" disabled={disabled || !Object.values(subscriptions).some(item => item.hasUrl && item.enabled)} onClick={() => void run('更新订阅', () => client.refresh())}>更新订阅</button></div>

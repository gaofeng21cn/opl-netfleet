...
interface PreviewSource { id: string; name: string; alias?: string | null; display_name?: string | null; nodeCount: number; pending: boolean; url?: string; user_agent?: string; info_url?: string; quota_reset_day?: number | null }
...
export function SubscriptionsPreview({ status, onResetDayChange }: { status: StatusSnapshot; onResetDayChange?: (id: string, day: number | null) => void }) {
  ...
  const sources = draftSources ?? (status.subscriptions || []).map((source) => ({ id: source.section, name: source.name || source.section, alias: source.alias, display_name: source.display_name, nodeCount: source.node_count || 0, pending: Boolean(source.pending_update), quota_reset_day: source.quota?.reset_day }));
  ...
  return <>
    ...
    {open && <div className="nf-managed-backdrop"><section className="nf-managed-dialog" role="dialog" aria-modal="true" aria-label="管理订阅">
      ...
      {adding || editing ? <form onSubmit={(event) => {
        ...
        const source = { id: editing?.id || String(values.get('id')), name: String(values.get('name')), alias: String(values.get('alias') || '').trim() || null, display_name: [String(values.get('name')).trim(), String(values.get('alias') || '').trim()].filter(Boolean).join(' · '), nodeCount: editing?.nodeCount || 0, pending, ...fields, quota_reset_day: resetDay ? Number(resetDay) : null };
        ...
        <div className="nf-form-rows">
          ...
          <label className="nf-form-row">账户备注（可选）<input name="alias" defaultValue={editing?.alias || ''} placeholder="例如：主账号" /></label>
        ...
        <table><thead><tr><th>名称</th><th>节点</th><th>流量重置日</th><th>状态</th><th>操作</th></tr></thead><tbody>{sources.map((source) => { const label = source.display_name || [source.name, source.alias].filter(Boolean).join(' · '); return <tr key={source.id}><td>{label}</td><td>{source.nodeCount}</td><td title="手动设置，仅供套餐参考；实际结算以机场为准">{quotaResetLabel(source.quota_reset_day) || '未设置'}</td><td>{source.pending ? '已保存，待更新订阅' : '已隐藏地址'}</td><td><button type="button" title="编辑" onClick={() => setEditing(source)}><Pencil aria-hidden="true" /></button><button type="button" title="更新" onClick={() => { if (window.confirm(`确认预览更新“${label}”？不会下载或写入设备。`)) setSources((items) => items.map((item) => item.id === source.id ? { ...item, pending: false } : item)); }}><RefreshCw aria-hidden="true" /></button><button type="button" title="删除" onClick={() => { if (window.confirm(`确认删除“${label}”？`)) setSources((items) => items.filter((item) => item.id !== source.id)); }}><Trash2 aria-hidden="true" /></button></td></tr> })}</tbody></table>

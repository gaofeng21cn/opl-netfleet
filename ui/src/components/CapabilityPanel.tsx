...
export function CapabilityPanel({ snapshot, capability, compact = false, onChooseRegion, onSelectAuto, onOpen, active, disabled = false }: {
  ...
  return <article className={`nf-exit-panel${compact ? ' is-compact' : ''}`}>
    ...
    <dl className="nf-exit-current"><div className="nf-exit-path"><dt>当前路径</dt><dd>{current}<small>{provider}</small></dd></div><div><dt>选择方式</dt><dd>{selection}{selectionNote && <small>{selectionNote}</small>}</dd></div><div><dt>延迟</dt><dd>{measured && !direct ? delay(capability.reason?.delay_ms) : '未测量'}</dd></div><div><dt>健康状态</dt><dd>{measured ? direct ? capability.data_path === 'direct_manual' ? '手动直连' : '直连退路' : capability.alive ? '健康' : '不可用' : '未测量'}</dd></div></dl>

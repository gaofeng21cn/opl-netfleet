...
export function OperationProgress({ operation, error, scope = '', subjectLabel, now = Date.now() / 1000 }: { operation: OperationSnapshot | null; error?: string | null; scope?: string; subjectLabel?: string; now?: number }) {
  ...
  const state = uncertain ? '连接或执行已中断，结果尚未确认' : active && operation.cancel_requested ? '已请求取消，等待当前安全步骤结束' : !active && operation.error?.startsWith('update_cancelled') ? '已取消' : !active && operation.error?.startsWith('update_deferred') ? '已延后' : operation.state === 'queued' ? '已提交，等待设备执行' : operation.state === 'running'
  ...
  const details = <>
      ...
      {operation.error && <><span className="nf-result-reason">{componentError(operation.error)}</span><details><summary>技术详情</summary><code>{operation.error}</code></details></>}
      {active && operation.kind === 'packages' && operation.subject !== 'feed' && <span>{operation.write_started ? '正在替换软件包，不能取消。请等待完成或恢复。' : '尚未替换软件包，可继续浏览；安全准备结束后才开始替换。'}</span>}
      {active && operation.can_cancel && <button type="button" disabled title="本机预览只读，请在设备 LuCI 中操作">{operation.cancel_requested ? '已请求取消' : '取消本次更新'}</button>}

import { ChevronRight, Clock3, Gauge, ShieldCheck, TimerReset } from 'lucide-react';
...
export function PolicySummary({ snapshot, collapsible = false }: { snapshot: StatusSnapshot; collapsible?: boolean }) {
  ...
  const grid = <div className="nf-policy-grid">
    {items.map(({ label, value, icon: Icon }) => (
      <dl key={label}><dt><Icon aria-hidden="true" />{label}</dt><dd>{value}</dd></dl>
    ))}
  </div>;
  // Desktop keeps the exit page about the current path; these reference values
  // stay in the page but wait behind a disclosure.
  if (collapsible) return <details className="nf-policy-summary is-collapsible">
    <summary><ChevronRight aria-hidden="true" /><span>运行口径</span><small>自动选优周期、切换门槛与失联保护 · 全部读取自当前设备策略</small></summary>
    {grid}
  </details>;
  return (

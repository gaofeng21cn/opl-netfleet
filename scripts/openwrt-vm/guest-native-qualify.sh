...
stage=management_benchmark
ucode /tmp/tests/benchmark-management.uc /usr/libexec/opl-netfleet >"$work/management-benchmark.json"
...
stage=concurrent_data_plane
sh /tmp/tests/native_workload.sh "$probe_port" "$work/workload" >"$work/workload-result.json"
assert_json "$work/workload-result.json" '@.ok' true
...
printf '{"ok":true,"scope":"native-mihomo-runtime","production_ready":false,"source_commit":"%s","source_tree":"%s","plugin_package_receipt":%s,"management_benchmark":%s,"checks":{"plugin_hot_loading":true,"plugin_core_pid_unchanged":true,"source_contracts":true,"native_subscription_crud":true,"subscription_download":true,"subscription_failed_cache_retained":true,"subscription_identity_isolation":true,"subscription_private_storage":true,"subscription_active_edit_guard":true,"subscription_same_content_mtime":true,"no_nikki":true,"shared_compile":true,"shared_enable":true,"shared_select":true,"shared_refresh":true,"shared_disable":true,"procd_owner":true,"controller":true,"owner_conflict_zero_mutation":true,"dual_stack_concurrent_transfer":true,"sustained_stream_and_status":true,"ipv4_negative_control":true,"ipv6_negative_control":true,"lan_ipv4_tcp":true,"lan_ipv4_udp":true,"lan_ipv6_tcp":true,"lan_ipv6_udp":true,"router_ipv4_tcp":true,"router_ipv4_udp":true,"router_ipv6_tcp":true,"router_ipv6_udp":true,"lan_ipv4_dns_tcp_udp":true,"lan_ipv6_dns_tcp_udp":true,"router_ipv4_dns_tcp_udp":true,"router_ipv6_dns_tcp_udp":true,"normal_stop_cleanup":true,"repeated_stop":true,"crash_recovery_or_cleanup":true,"direct_after_stop":true,"direct_after_crash":true,"invalid_config_no_interception":true,"foreign_rules_preserved":true}}\n' "$commit" "$tree" "$plugin_package_receipt" "$(cat "$work/management-benchmark.json")"

...
apk update >&2
apk add python3 python3-pip libstdcpp ca-bundle ip-full kmod-veth >&2
...
print(json.dumps({"ok": True, "source_commit": sys.argv[1], "source_tree": sys.argv[2],
                  "checks": {"musl_runtime": True, "protocol_wire": True, "kernel_lease": True}, "production_ready": False}))
PY

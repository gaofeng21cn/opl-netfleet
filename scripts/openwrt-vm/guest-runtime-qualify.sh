...
write_config_request() {
	...
	cat >"$output" <<EOF
{"request":{"revision":"$revision","policy_source":{"kind":"$policy_source_kind","ref":"$policy_source_ref"},"recovery_profile_ref":"subscription:base","providers":{"alpha":{"enabled":true,"role":"primary","billing":"subscription"},"beta":{"enabled":true,"role":"reserve","billing":"buyout"}},"regions":{"japan":{"display_name":"$japan_display_name","mode":"automatic"},"singapore":{"display_name":"Singapore","mode":"automatic"}},"capabilities":{"standard":{"enabled":true,"mode":"automatic","region_ids":["japan","singapore"]}},"automation":{"enabled":true,"selection_interval_seconds":$selection_interval,"subscription_refresh_enabled":true,"subscription_refresh_interval_seconds":3600},"safety":{"region_switch_margin_ms":150,"leaf_switch_margin_ms":150,"runtime_grace_seconds":30,"latency_url":"$probe_url","path_probe_url":"$probe_url","guard_probe_url":"$probe_url"}}}
EOF
}
